...
*   **Gateway Central Registry**: The MCP Gateway also subscribes to `/mcp/directory` and caches all server entries. Late-joining Agents call `/mcp/gateway/list_servers` once at startup to discover servers that published before the Agent joined — combining push (subscription) and pull (one-time query) for reliability.
