...
| `/mcp/directory` | _（无）_ | MCP 服务器发现。`MCPServerDescription` 由 `MCPServerComponent` 和 Gateway 在激活时发布。Agent 订阅后自动发现；晚加入的 Agent 通过 `/mcp/gateway/list_servers` 拉取。 | MCPServerComponent / Gateway |
