...
- [x] ~~**MCP Transport Layer**~~: Removed in tagentacle-py-mcp v0.2.0 — replaced by direct Streamable HTTP connections.
- [x] **MCPServerComponent**: `tagentacle-py-mcp` v0.2.0 — composable MCP server mixin for `LifecycleNode` with auto Streamable HTTP + `/mcp/directory` publishing.
...
- [x] **TACL (Tagentacle Access Control Layer)**: `tagentacle-py-mcp` v0.3.0 — MCP-level JWT authentication with `auth_required` on `MCPServerComponent`, `AuthMCPClient`, `TACLAuthority` (SQLite agent registry + credential issuer).
...
- [x] **JSON Schema Validation**: `tagentacle-py-core` v0.3.0 — `SchemaRegistry` with auto-discovery from interface packages, configurable per-node (`strict`/`warn`/`off`), integrated into `Node.publish()` and `Node._dispatch()`. Requires optional `jsonschema>=4.0`.
- [x] **TACL `space` Claim**: `tagentacle-py-mcp` v0.4.0 — JWT `space` field binding agents to isolated execution environments. Full stack: `CallerIdentity.space`, `sign_credential(space=...)`, `TACLAuthority.register_agent(space=...)`.
...
- [x] **Shell Server Pkg**: `shell-server` v0.1.0 — `LifecycleNode` + `MCPServerComponent` exposing `exec_command` tool with TACL `space`-aware dynamic container routing (JWT → container → local fallback).
