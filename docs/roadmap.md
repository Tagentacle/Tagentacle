...
- [x] **MCPServerNode Base Class** (now MCPServerComponent): `tagentacle-py-mcp` v0.2.0 — base class for MCP Server Nodes with auto Streamable HTTP + `/mcp/directory` publishing.
...
- [x] **TACL (Tagentacle Access Control Layer)**: `tagentacle-py-mcp` v0.3.0 — MCP-level JWT authentication with `auth_required` on MCPServerNode (now MCPServerComponent), `AuthMCPClient`, `PermissionMCPServerNode` (now TACLAuthority) (SQLite agent registry + credential issuer).
